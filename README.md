...
This operator provides an OpenAI-compatible chat interface with two modes:

**Analyze Mode** - Analyze existing media samples with custom prompts:

```py
# Analyze media with natural language prompts
# The operator handles file uploads automatically and uses file references
from vlmrun import VLMRun

client = VLMRun(api_key="your-api-key", base_url="https://agent.vlm.run/v1")

# Upload file and get file_id
uploaded = client.files.upload(file="path/to/image.jpg")
...
**Generate Mode** - Generate new content from text prompts (no input media required):

```py
# Generate content from a text prompt
response = client.agent.completions.create(
    model="vlmrun-orion-1:pro",
    messages=[
        {"role": "system", "content": "You are a content generation assistant. Always return an asset."},
        {"role": "user", "content": "Create a 5-second video of a moon landing"}
    ]
)
# Returns artifact references (e.g., vid_abc123, img_xyz789) that can be downloaded
```
...
**Response Formats (Analyze mode only):**
- **Text** - Free-form text responses (can include generated images/videos)
- **JSON Object** - Flexible JSON output for structured data

**Features:**
- **Two modes**: Analyze existing media or generate new content
- Custom natural language prompts for any task
- Optional system prompts for context setting
- Temperature control for response creativity
- Token usage tracking
- **Save Generated Artifacts** - Artifacts generated by the model (images, videos,
  audio, documents) are automatically saved and added as new samples to your
  dataset with the `vlmrun_generated` tag
